...
export function StaffClient({ initialStaff }: StaffClientProps) {
  ...
  return (
    <div className="space-y-6 p-6 md:p-10 text-zinc-950">
      {/* Title & Headers */}
      <div className="flex flex-col gap-4 border-zinc-200 border-b pb-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-zinc-500">
            Manage operator profiles, activity streams, and daily shift statistics.
          </p>
        </div>
        <Button
        ...
        <CardContent className="p-0">
          {initialStaff.length === 0 ? (
            <div className="p-12 text-center text-zinc-450">
              ...
              <p className="mt-1 text-xs text-zinc-505">Add your first operator to get started.</p>
              <Button className="mt-4 bg-black text-white" onClick={() => setIsOpen(true)}>
                + Add Staff
              </Button>
            </div>
          ) : (
            <Table>
              <TableHeader className="bg-zinc-50 border-zinc-150 border-b">
                <TableRow>
                  <TableHead className="font-bold text-zinc-500 uppercase tracking-wider text-[10px]">Name</TableHead>
                  <TableHead className="font-bold text-zinc-500 uppercase tracking-wider text-[10px]">Email</TableHead>
                  <TableHead className="font-bold text-zinc-500 uppercase tracking-wider text-[10px]">Role</TableHead>
                  <TableHead className="font-bold text-zinc-500 uppercase tracking-wider text-[10px]">Joined Date</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {initialStaff.map((item) => (
                  <TableRow key={item.id} className="border-zinc-150 border-b hover:bg-zinc-50/20">
                    <TableCell className="font-bold text-zinc-900 text-sm">{item.name}</TableCell>
                    <TableCell className="text-zinc-700 text-xs">{item.email}</TableCell>
                    <TableCell>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="bg-white border-zinc-200 max-w-md">
          <DialogHeader>
            <DialogTitle className="font-extrabold text-lg text-zinc-950">Add Staff Operator</DialogTitle>
            <DialogDescription className="text-zinc-550 text-sm">
              Create a new operator account for your pharmacy. Specify their login password directly.
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g. Amit Kumar" className="border-zinc-200 focus:border-black" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input type="email" placeholder="e.g. amit@example.com" className="border-zinc-200 focus:border-black" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <DialogFooter className="gap-2 pt-4">
                ...
                <Button
                  ...
                  disabled={isLoading}
                >
                  {isLoading ? <Loader2 className="size-4 animate-spin" /> : "Add Staff"}
                </Button>
              </DialogFooter>
            </form>
          </Form>
